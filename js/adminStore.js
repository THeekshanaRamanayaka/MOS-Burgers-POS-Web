    ...
    if (productList[category]) {
        productList[category].forEach((product, index) => {
            ...
            productCard.innerHTML = `
                <div class="card product-card align-items-center" onclick="addToTextField(${index})">
...
function addToTextField(index) {
    ...
    if (product) {
        ...
        // document.getElementById('itemImage').value = product.img;
        document.getElementById('itemDiscount').value = product.discount;
    }
}
...
function addItem() {
    ...
    if (itemCode && itemName && itemPrice && itemDiscount && itemImage) {
        productList[currentCategory].push({
            itemCode: itemCode,
            name: itemName,
...
function updateItem() {
    ...
    if (itemCode && itemName && itemPrice && itemDiscount && itemImage) {
        ...
        productList[currentCategory].forEach((product, index) => {
            if (product.itemCode === itemCode) {
                productList[currentCategory][index] = updatedProduct;
            }
        });
        renderProductList(currentCategory);
        clearInputFields();
...
function deleteItem() {
    ...
    if (itemCode) {
        ...
        for (let category in productList) {
            const index = productList[category].findIndex(item => item.itemCode === itemCode);
            if (index !== -1) {
